...
#
# Bash completion generated for 'npp' at Tue Aug 07 2018 18:06:39 GMT-0700 (Pacific Daylight Time).
#
...
#   1. Uncomment the "_npp_log_file=..." line.
#   2. 'tail -f /var/tmp/dashdash-completion.log' in one terminal.
#   3. Re-source this bash completion file.
#_npp_log=/var/tmp/dashdash-completion.log

function _npp_completer {

    # ---- cmd definition

    local cmd_shortopts="-a -a -f -h -h -h -v -v -v"
local cmd_longopts="--all --all --allow-unknown --allow-unknown --allow-unknown --force --help --help --help --release --verbosity --verbosity --verbosity --version --view-all --view-all --view-full-path --view-git-stash --view-git-stash --view-npm-registry --view-npm-registry --view-packages-path --view-packages-path --view-path --view-path --view-registry --view-registry --view-stash --view-stash --vn"
local cmd_optargs="--release=string --verbosity=integer --verbosity=integer --verbosity=integer -v=integer -v=integer -v=integer"
...
if type complete &>/dev/null; then
    function _npp_completion {
        local _log_file=/dev/null
        [[ -z "$_npp_log" ]] || _log_file="$_npp_log"
        COMPREPLY=($(COMP_CWORD="$COMP_CWORD" \
            COMP_LINE="$COMP_LINE" \
            COMP_POINT="$COMP_POINT" \
            _npp_completer -- "${COMP_WORDS[@]}" \
            2>$_log_file)) || return $?
    }
    complete -o default -F _npp_completion npp
    ...
    function _npp_completion {
        ...
        compadd -- $(COMP_CWORD=$((CURRENT-1)) \
            COMP_LINE=$BUFFER \
            COMP_POINT=0 \
            _npp_completer -- "${words[@]}" \
            2>$_log_file)
    }
    compdef _npp_completion npp
    ...
    compctl -K _npp_completion npp
fi


##
## This is a Bash completion file for the 'npp' command. You can install
## with either:
##
##     cp FILE /usr/local/etc/bash_completion.d/npp   # Mac
##     cp FILE /etc/bash_completion.d/npp             # Linux
##
## or:
##
##     cp FILE > ~/.npp.completion
##     echo "source ~/.npp.completion" >> ~/.bashrc
##
